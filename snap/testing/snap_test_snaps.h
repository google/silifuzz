...
extern const SnapCorpus<Host> kSnapGeneratorTestCorpus;
extern const SnapCorpus<Host> kSnapRunnerTestCorpus;

// Returns a Snap generator test Snap of the given type.
inline const Snap<Host>& GetSnapGeneratorTestSnap(SnapGeneratorTestType type) {
  const Snap<Host>* snap = kSnapGeneratorTestCorpus.Find(EnumStr(type));
  if (snap == nullptr) {
...
inline const Snap<Host>& GetSnapRunnerTestSnap(TestSnapshot type) {
