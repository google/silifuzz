...
// Counting zeros in a mask is error prone, cross-check the bits in the mask.
static_assert(kPStateMask & (1ULL << 31), "N is not in mask.");
static_assert(kPStateMask & (1ULL << 30), "Z is not in mask.");
static_assert(kPStateMask & (1ULL << 29), "C is not in mask.");
static_assert(kPStateMask & (1ULL << 28), "V is not in mask.");
...
void ConvertGRegsFromLibC(const ucontext_t& libc_ucontext,
  ...
  // Linux will leak pstate bits from SPSR_EL1 that cannot be set in EL0. Zero
  // all the bits that we cannot save and restore.
  // If the pstate bits generated by SaveUContext end up having similar
  // problems, for some reason, register masking should be moved into
  // ZeroOutGRegsPadding.
  gregs->pstate = mcontext.pstate & kPStateMask;
