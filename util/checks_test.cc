...
TEST(ChecksTest, Log) {
  ...
  EXPECT_DEATH_IF_SUPPORTED({ LOG_FATAL_IF(true, "fatal if on ", 88); },
                            "fatal if on 88");
  if (DEBUG_MODE) {
    EXPECT_DEATH_IF_SUPPORTED({ LOG_DFATAL("dfatal ", IntStr(99)); },
                              "dfatal 99");
    LOG_DFATAL_IF(false, "dfatal if off ", 111);
    EXPECT_DEATH_IF_SUPPORTED({ LOG_DFATAL_IF(true, "dfatal if on ", 111); },
                              "dfatal if on 111");
  } else {
...
TEST(ChecksTest, Check) {
  ...
  ABSL_ATTRIBUTE_UNUSED bool my_false_a = false;
  ABSL_ATTRIBUTE_UNUSED bool my_false_b = false;
  EXPECT_DEATH_IF_SUPPORTED({ CHECK(my_false_a); }, "Check failed: my_false_a");
  EXPECT_DEATH_IF_SUPPORTED({ CHECK_LE(22, 11); }, "Check failed: 22 <= 11");
  if (DEBUG_MODE) {
    EXPECT_DEATH_IF_SUPPORTED({ DCHECK(my_false_b); },
                              "Check failed: my_false_b");
...
TEST(ChecksTest, ASSLog) {
  ...
  EXPECT_DEATH_IF_SUPPORTED({ ASS_LOG_FATAL("fatal ", IntStr(55)); },
                            "fatal 55");
  if (DEBUG_MODE) {
    EXPECT_DEATH_IF_SUPPORTED({ ASS_LOG_DFATAL("dfatal ", IntStr(66)); },
                              "dfatal 66");
  } else {
...
TEST(ChecksTest, ASSCheck) {
  ...
  ABSL_ATTRIBUTE_UNUSED bool my_false_a = false;
  ABSL_ATTRIBUTE_UNUSED bool my_false_b = false;
  EXPECT_DEATH_IF_SUPPORTED({ ASS_CHECK(my_false_a); },
                            "Check failed: my_false_a");
  if (DEBUG_MODE) {
    EXPECT_DEATH_IF_SUPPORTED({ ASS_DCHECK(my_false_b); },
                              "Check failed: my_false_b");
  } else {
    ASS_DCHECK(my_false_b);
  }
}
...
TEST(ChecksTest, CheckAndUse) {
  ...
  ABSL_ATTRIBUTE_UNUSED bool my_false_a = false;
  ABSL_ATTRIBUTE_UNUSED bool my_false_b = false;
  EXPECT_DEATH_IF_SUPPORTED({ v = CHECK_AND_USE(my_false_a, 33); },
                            "Check condition failed: my_false_a");
  if (DEBUG_MODE) {
    EXPECT_DEATH_IF_SUPPORTED({ v = DCHECK_AND_USE(my_false_b, 44); },
                              "Check condition failed: my_false_b");
  } else {
    v = DCHECK_AND_USE(my_false_b, 44);
    CHECK_EQ(v, 44);
  }
}
