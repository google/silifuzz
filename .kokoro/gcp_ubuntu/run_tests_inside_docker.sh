...
# Remove absolute convenience symlinks generated by Bazel inside Docker on exit
# as Kokoro Placer fails if build output contains symlinks to absolute paths.
trap 'rm -f bazel-*' EXIT
